//
//  GroupAdminPanelTableViewController.swift
//  Pigeon-project
//
...
class GroupAdminPanelTableViewController: UITableViewController {
  ...
  fileprivate func setupColorsAccordingToTheme() {
    groupProfileTableHeaderContainer.profileImageView.layer.borderColor = ThemeManager.currentTheme().generalSubtitleColor.cgColor
    groupProfileTableHeaderContainer.userData.layer.borderColor = ThemeManager.currentTheme().generalSubtitleColor.cgColor
  ...
  func addMembers() {
    let filteredMemebrs = RealmKeychain.realmUsersArray().filter { user in
      ...
      if members[indexPath.row].id == Auth.auth().currentUser?.uid {
        ...
        if let statusString = members[indexPath.row].onlineStatus as? String {
          if statusString == statusOnline {
              cell.subtitle.textColor = .systemBlue
